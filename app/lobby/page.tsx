...
import { getProgram, getGamePDA, getPlayerPDA, CONNECTION, TREASURY } from '@/lib/program';
...
export default function Lobby() {
  ...
  const ensurePlayerProfile = async () => {
    ...
    try {
      ...
      Buffer.from(publicKey.toString().slice(0, 16)).copy(username);
  ...
  const handleCreateGame = async () => {
    ...
    try {
      ...
      const wagerLamports = new BN(Math.floor(parseFloat(wagerInput) * LAMPORTS_PER_SOL));

      await program.methods
        .createGame(wagerLamports)
        .accounts({
          creator: publicKey,
          playerAccount: getPlayerPDA(publicKey)[0],
          game: gamePDA,
          treasury: TREASURY,
          systemProgram: SystemProgram.programId,
        })
      ...
      localStorage.setItem('gameCreator', publicKey.toString());
  ...
  const handleJoinGame = async (game: any) => {
    ...
    try {
      ...
      const program = getProgram(anchorWallet);
      const creatorPubkey = game.creatorKey;
      const [gamePDA] = getGamePDA(creatorPubkey);
      const [playerPDA] = getPlayerPDA(publicKey);
      ...
      localStorage.setItem('gameCreator', creatorPubkey.toString());
      setStatus('Joined!');
      router.push('/game');
    } catch (e: any) {
  ...
  const mockGames = [
    { id: 1, creator: 'Dh3k...9mPq', creatorKey: publicKey, wager: 0.5 * LAMPORTS_PER_SOL, wagerSol: '0.5' },
    { id: 2, creator: 'Bx7r...4kLm', creatorKey: publicKey, wager: 1.0 * LAMPORTS_PER_SOL, wagerSol: '1.0' },
    { id: 3, creator: 'Yz2s...8nWq', creatorKey: publicKey, wager: 0.1 * LAMPORTS_PER_SOL, wagerSol: '0.1' },
  ];
  ...
  return (
    <main style={{ minHeight: '100vh', position: 'relative' }}>
      ...
      <nav style={{
        ...
        padding: '20px 48px', borderBottom: '1px solid rgba(42,106,155,0.2)',
        backdropFilter: 'blur(10px)', zIndex: 100, background: 'rgba(2,12,20,0.8)',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }}
          onClick={() => router.push('/')}>
          <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#00ff88', boxShadow: '0 0 10px #00ff88' }} />
          <span className="font-display" style={{ fontSize: '20px', letterSpacing: '4px' }}>BEAT THE VESSEL</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
          {publicKey && (
            <div className="font-mono" style={{ color: '#6a9ab8', fontSize: '12px', letterSpacing: '1px', background: 'rgba(42,106,155,0.1)', padding: '6px 12px', border: '1px solid rgba(42,106,155,0.3)' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '120px 48px 80px', position: 'relative', zIndex: 1 }}>
        <div style={{ marginBottom: '48px' }}>
          <div className="font-mono" style={{ color: '#00ff88', fontSize: '11px', letterSpacing: '4px', marginBottom: '12px' }}>◆ COMMAND CENTER</div>
        ...
        {error && (
          <div style={{ padding: '16px', marginBottom: '24px', background: 'rgba(255,51,51,0.1)', border: '1px solid rgba(255,51,51,0.4)', color: '#ff3333' }} className="font-mono">
            ⚠ {error}
          </div>
        )}

        {status && (
          <div style={{ padding: '16px', marginBottom: '24px', background: 'rgba(0,255,136,0.1)', border: '1px solid rgba(0,255,136,0.4)', color: '#00ff88' }} className="font-mono">
            ◆ {status}
          </div>
        )}
        ...
        {tab === 'create' && (
          <div className="panel" style={{ padding: '40px' }}>
            <h2 className="font-display" style={{ fontSize: '32px', letterSpacing: '4px', marginBottom: '8px' }}>DEPLOY YOUR FLEET</h2>
            <p style={{ color: '#6a9ab8', marginBottom: '32px', fontSize: '15px' }}>Set your wager. Your SOL will be locked until the battle ends.</p>

            <div style={{ marginBottom: '24px' }}>
              <label className="font-mono" style={{ display: 'block', fontSize: '11px', letterSpacing: '3px', color: '#6a9ab8', marginBottom: '12px' }}>
                WAGER AMOUNT (SOL)
              </label>
              <div style={{ position: 'relative' }}>
                <input type="number" value={wagerInput} onChange={(e) => setWagerInput(e.target.value)}
                  ...
                  style={{
                    ...
                    background: 'rgba(6,32,48,0.8)', border: '1px solid rgba(42,106,155,0.4)',
                    color: '#00ff88', fontFamily: 'Share Tech Mono, monospace',
                    fontSize: '32px', outline: 'none', letterSpacing: '2px',
                  }} />
                <span className="font-display" style={{ position: 'absolute', right: '20px', top: '50%', transform: 'translateY(-50%)', color: '#6a9ab8', fontSize: '20px' }}>SOL</span>
              </div>
            ...
            <button onClick={handleCreateGame} disabled={creating} className="btn-primary" style={{ width: '100%', fontSize: '22px' }}>
              {creating ? `⚓ ${status || 'PROCESSING...'}` : '⚓ CREATE BATTLE'}
            </button>
          </div>
        )}
              ...
              {mockGames.map((game) => (
                ...
                >
                  ...
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div className="font-display" style={{ color: '#00ff88', fontSize: '32px', textShadow: '0 0 10px rgba(0,255,136,0.4)' }}>{game.wagerSol} SOL</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div className="font-mono" style={{ color: '#6a9ab8', fontSize: '11px', letterSpacing: '2px', marginBottom: '4px' }}>YOU WIN</div>
                    <div className="font-display" style={{ color: '#ffc400', fontSize: '24px' }}>{(parseFloat(game.wagerSol) * 2 * 0.9).toFixed(2)} SOL</div>
                  </div>
